...
@Service
public class OrganizationServiceImpl extends BaseServiceImpl<Organization, VOOrganization>
    ...
    protected boolean saveBefore(VOOrganization organization, boolean isAdd) {
        if (isAdd) {
            Organization po = BeanUtils.poVo(organization, poClass, "");
            generateCode(po);
        }
    ...
    @Override
    protected boolean saveAfter(Organization organization, boolean beforeState, boolean isAdd) {
        if (beforeState) {
            TreeNode root = loadRoot();
            if (isAdd) {
    ...
    private void generateCode(Organization organization) {
        Page page = new Page();
        page.setRows(32 * 32);
        ...
        if (result == null) {
            ...
            ((List<Organization>) result).forEach(o -> {
    ...
    private TreeNode loadRoot() {
        ...
        @SuppressWarnings("unchecked")
        List<Organization> organizations = (List<Organization>) page.getResult();
        // 创建根节点
        createRoot(treenode);
        // 装载子节点
    ...
    private String getParentIndex(Organization o, Map<String, Organization> map, Set<String> indexCache) {
        Organization po = map.get(o.getPcode());
        if (po == null)
            return "R";
     ...
     * 装载子节点
     */
    private void loadSons(TreeNode rootNode, List<Organization> list) {
        ...
        Map<String, Organization> map = list.stream().collect(Collectors.toMap(k -> k.getCode(), v -> v));
     ...
     * 拷贝节点信息
     */
    private void copy(Organization src, TreeNode dis) {
    ...
    @Override
    public BaseDao<Organization, Serializable> getBaseDao() {
        return organizationDao;
    }
